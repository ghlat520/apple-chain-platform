...
// ========== 采集回调（内部接口） ==========
export const jobCallbackApi = {
  jobRunCallback: (data) => request.post('/bigdata/job/run/callback', data),
}

// ========== 运维监控 ==========
export const opsApi = {
  getOpsHealth: () => request.get('/bigdata/ops/health'),
  getOpsMetrics: (params) => request.get('/bigdata/ops/metrics', { params }),
}

// ========== 角色权限 ==========
export const roleApi = {
  listRoles: (params) => request.get('/bigdata/role', { params }),
  createRole: (data) => request.post('/bigdata/role', data),
  updateRole: (id, data) => request.put(`/bigdata/role/${id}`, data),
  deleteRole: (id) => request.delete(`/bigdata/role/${id}`),
  getRolePermissions: (params) => request.get('/bigdata/role/permissions', { params }),
  assignRolePermissions: (data) => request.post('/bigdata/role/permissions', data),
  removeRolePermission: (id) => request.delete(`/bigdata/role/permissions/${id}`),
  getRolePermissionList: (roleCode) => request.get(`/bigdata/role/${roleCode}/permissions`),
  updateRolePermissionList: (roleCode, data) => request.post(`/bigdata/role/${roleCode}/permissions`, data),
}
